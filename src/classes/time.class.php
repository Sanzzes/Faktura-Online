...
class timestamp {
    ...
    function timestamp_german2mysql($date) {
        ...
        $timestamp = sprintf("%04d-%02d-%02d", 
                ...
                substr($split[0], 3, 2), 
                substr($split[0], 0, 2));
        $timedate = strtotime($timestamp);
        return $timedate;
    }
    
    function timesplitt($date) {
                $split = explode(" ", $date);
        $timestamp = sprintf("%04d-%02d-%02d", 
                substr($split[0], 6, 4), 
                substr($split[0], 3, 2), 
    ...
    function timestamp_time2mysql($time) {
        ...
        $split = explode(" ", $time);
        $timemysql = sprintf("%02d%02d", substr($split[0], 0, 2), substr($split[0], 3, 2));
    ...
    function roundTime($time) {
        ...
        $timeround = sprintf("%02d Std %02d Min.", substr($split[0], 0, 2), substr($split[0], 2, 2));
        return $timeround;
    }
